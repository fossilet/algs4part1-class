...
public class DequeTest extends TestCase
{
    ...
    // Statics
    private static final int MAX_PROBLEM_SIZE = 10000;
    ...
    public void testIsEmptyAfterMultipleAddRemove( )
    {
        ...
        assertTrue("Should be empty after adding and removing " + MAX_PROBLEM_SIZE + " elements.", mDequeue.isEmpty());
    }
    ...
    public void testMultipleIterator( )
    {
        for (int i = 0; i < MAX_PROBLEM_SIZE / 1000; i++) {
            ...
            Iterator[ ] someIts = { mDequeue.iterator(), mDequeue.iterator(), mDequeue.iterator(), mDequeue.iterator(),
                    mDequeue.iterator(), mDequeue.iterator() };

            @SuppressWarnings("unchecked")
            Iterator<String>[ ] manyStrIt = (Iterator<String>[ ]) someIts;

            for (int iterID = 0; iterID < manyStrIt.length; iterID++) {
                int index = 0;
                while (manyStrIt[iterID].hasNext()) {
                    assertEquals("Iterator #" + iterID + " failed:\n", String.valueOf(index), manyStrIt[iterID].next());
                    index++;
                }
            }
